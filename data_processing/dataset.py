...
#         return np.random.choice(self.noise_filenames)
    ...
    def _gen_noise_stft(self, clean_mag, snr = 0):
        k = 10 ** (- snr / 20 )
        #amp = np.max(clean_mag)
        #power = np.sum(clean_mag ** 2)
        shape = clean_mag.shape
        mask = np.ones(shape)
        mask = np.squeeze(mask)
        mask = mask + k * np.random.standard_normal(shape)
        mask = mask * np.random.choice((0,1), size=shape, p = [0.10, 0.90])
#         i, j = np.random.randint(8, size=2)
#         mask[:i, :] = 0
#         mask[-j:, :] = 0
        noise_mag = mask * clean_mag
        return noise_mag
    ...
    def parallel_audio_processing(self, clean_filename):
        ...
        return noise_magnitude, clean_magnitude, clean_phase #noise_phase
